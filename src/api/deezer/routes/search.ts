...
const globalSearch = async (query: string, config?: Config) => {
  ...
  return await fetchResponseHandler<SuccessResponse<DeezerSearchResponse[]>>(
    `${URL}${queryString}'`,
    fetchConfig,
  );
};
...
const album = async (searchString?: string, fetchConfig?: RequestInit) => {
  ...
  return await fetchResponseHandler<SuccessResponse<Album[]>>(
    `${URL}/album/${queryString}`,
    fetchConfig,
  );
};
...
const artist = async (searchString?: string, fetchConfig?: RequestInit) => {
  ...
  return await fetchResponseHandler<SuccessResponse<Artist[]>>(
...
const track = async (searchString?: string, fetchConfig?: RequestInit) => {
  ...
  return await fetchResponseHandler<SuccessResponse<Track[]>>(
    `${URL}/track/${queryString}`,
    fetchConfig,
  );
};
